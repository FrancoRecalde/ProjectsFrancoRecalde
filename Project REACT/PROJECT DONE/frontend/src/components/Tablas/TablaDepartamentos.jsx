...
import { COLUMNS_DEPARTAMENTOS } from "../data/columns.jsx";

export const TablaDepartamentos = () => {
    const [data, setData] = useState([]);
    const [region, setRegion] = useState([]);
    const COLUMNS = useMemo(() => COLUMNS_DEPARTAMENTOS, []);
    ...
    const filterSubmit = async (data) => {
        try {
            departamentosService
                .getDeptosFiltradas(data)
                .then((data) => setData(data));
        } catch (error) {
            console.error("Error fetching filtered deptos:", error);
        }
    };
    ...
    const onSubmit = async (data) => {
        console.log(data);
        try {
            await departamentosService.updatDepto(data);
            departamentosService.getDeptos().then((data) => setData(data));
            setAccordion(-1);
        } catch (e) {
    ...
    useEffect(() => {
        departamentosService.getDeptos().then((response) => setData(response));
        getData();
    }, []);

    const eliminar = async (depto) => {
        try {
            await departamentosService.deleteDepto(depto);
            departamentosService.getDeptos().then((data) => setData(data));
            setModal(false);
        } catch (e) {
